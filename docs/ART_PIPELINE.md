...
Existing trained policies and seeded terrain outputs are unchanged.

The environment combines two CC0 photographic material scans with procedural
variation, shared material weights and the existing GGX terrain lighting.
Texture height drives surface-gradient normals without displacement. Snow keeps
its finer procedural treatment; paths suppress relief. Temperate broadleaf trees
have fuller crowns and low groundcover, with subtle shader leaf motion. Props
...
occlusion remain intact during cutaways.
...
and sensors. Vegetation is still generated geometry; its centimetre-scale motion
uses static shadows and cached reflections. Moving units do not appear in the
