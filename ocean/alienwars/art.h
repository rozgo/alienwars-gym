...
static void aw_art_close(void){
    if(!aw_art_ready)return;for(int i=0;i<4;i++)UnloadMesh(aw_art_mesh[i]);
    MemFree(aw_art_material.maps);aw_art_ready=0;
}
static void aw_art_nursery(AwBuilder*b,Vector3 p,int side){
    aw_art_init();const Mesh*m=&aw_art_mesh[3];aw_reserve(b,m->vertexCount);
