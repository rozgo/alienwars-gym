...
static int aw_navigation_replan(AwMissionWorld*w,const AwMap*m,int id){
    ...
    for(int n=0;n<g.nodes;n++){
        ...
        for(int j=0;j<w->count&&clear;j++)if(j!=id&&a->tracks[j].valid){
            ...
            if(fabsf(p.y-probe.position.y)>(own.height+other.height)*.5f+.2f)continue;
            if(hypotf(p.x-probe.position.x,p.z-probe.position.z)<hypotf(own.width,own.length)+hypotf(other.width,other.length)+.3f)clear=0;
        }
