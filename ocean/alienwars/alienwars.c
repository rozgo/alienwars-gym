...
AW_EXPORT const char*aw_fleet_snapshot(void){
    ...
    if(fleet.ready)for(int i=0;i<AW_UNITS;i++){
        const AwMissionAgent*a=&fleet.unit[i];n+=snprintf(json+n,sizeof(json)-n,
          "%s{\"id\":%d,\"family\":%d,\"active\":%d,\"arrivals\":%d,\"contacts\":%d,\"collisions\":%d,\"terrain_contacts\":%d,\"unit_contacts\":%d,\"patrol_restarts\":%d,\"global_retries\":%d,\"confined_ticks\":%d,\"remaining\":%.3f,\"timeout\":%d,\"failed\":%d,\"replans\":%d,\"deadlocks\":%d,\"assists\":%d,\"position\":[%.3f,%.3f,%.3f]}",
          i?",":"",i,a->vehicle.family,fleet.active[i],fleet.arrivals[i],fleet.total_contacts[i],fleet.total_collisions[i],fleet.total_terrain[i],fleet.total_units[i],fleet.patrol_restarts[i],fleet.recovery_attempts[i],a->deadlock_ticks,a->remaining,a->timeout,a->vehicle.failed,a->replans,a->deadlock_events,a->safety_interventions,a->vehicle.position.x,a->vehicle.position.y,a->vehicle.position.z);
    }
