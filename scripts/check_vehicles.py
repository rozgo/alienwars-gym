...
report={}
for name in ['vehicle','patrol']:
    source=f'tests/alienwars/{name}_test.c'
    subprocess.run(['clang','-std=c11','-O1','-g','-fsanitize=address,undefined','-I.',source,'-lm','-o',f'build/{name}-test'],check=True)
    native=subprocess.check_output([f'build/{name}-test'],text=True)
    subprocess.run([emcc,'-std=c11','-O3','-I.',source,'-lm','-sSTACK_SIZE=2MB','-sALLOW_MEMORY_GROWTH=1','-sASSERTIONS=1','-sENVIRONMENT=node','-o',f'build/{name}-test.js'],check=True)
