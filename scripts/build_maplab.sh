...
report={'generator_version':13,'navigation_contract':json.loads(pathlib.Path('build/web/alienwars/contract.json').read_text())['contract'],'source_commit':subprocess.check_output(['git','rev-parse','HEAD'],text=True).strip(),
