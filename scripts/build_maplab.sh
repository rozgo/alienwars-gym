...
paths=['build.sh','ocean/alienwars','vendor/flecs','web/maplab','scripts/build_maplab.sh','scripts/build_fleet_site.py','web/training']
...
report={'generator_version':11,'source_commit':subprocess.check_output(['git','rev-parse','HEAD'],text=True).strip(),
    ...
    'raylib':'5.5','flecs':json.loads(pathlib.Path('vendor/flecs/version.json').read_text()),'artifacts':artifacts}
