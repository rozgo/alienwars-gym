#!/usr/bin/env python3
"""Fetch two CC0 Poly Haven scans and pack RGB albedo + height in alpha.
...
manifest={}
for name,stem in [('forest_ground_04','forest'),('rock_face_03','rock')]:
    ...
    for channel in ['Diffuse','Displacement']:
        ...
        channels[channel]=Image.open(io.BytesIO(data)).resize((512,512),Image.Resampling.LANCZOS)
    ...
    manifest[stem]={'asset':name,'source':'https://polyhaven.com/a/'+name,'license':'CC0-1.0','inputs':inputs,'packed':'RGB albedo (sRGB), alpha height (linear), 512x512','sha256':hashlib.sha256(path.read_bytes()).hexdigest(),'bytes':path.stat().st_size}
