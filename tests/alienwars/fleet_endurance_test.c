...
int main(int argc,char**argv){
    ...
    for(int i=0;i<12;i++)printf("{\"unit\":%d,\"family\":%d,\"active\":%d,\"arrivals\":%d,\"contacts\":%d,\"collisions\":%d,\"timeout\":%d,\"failed\":%d,\"remaining\":%.3f,\"global_retries\":%d,\"confined_ticks\":%d,\"terrain_contacts\":%d,\"unit_contacts\":%d}\n",i,fleet.unit[i].vehicle.family,fleet.active[i],fleet.arrivals[i],fleet.total_contacts[i],fleet.total_collisions[i],fleet.unit[i].timeout,fleet.unit[i].vehicle.failed,fleet.unit[i].remaining,fleet.recovery_attempts[i],fleet.unit[i].deadlock_ticks,fleet.total_terrain[i],fleet.total_units[i]);
