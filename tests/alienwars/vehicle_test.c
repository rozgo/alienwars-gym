...
int main(){
 assert(aw_vehicle_spec(0,0).speed>aw_vehicle_spec(0,1).speed);
 assert(aw_vehicle_spec(0,0).speed*aw_ground_traction(0,AW_MUD)<aw_vehicle_spec(0,1).speed*aw_ground_traction(1,AW_MUD));
 assert(aw_vehicle_sensor_range(AW_VEHICLE_WING,2)>aw_vehicle_sensor_range(AW_VEHICLE_WING,1));
 ...
 AwVehicle boat={.family=AW_VEHICLE_BOAT,.variant=1,.position={-20,-.12f,64}};AwBody neighbor={{-18.8f,.1f,64},{0},0,.3f,1,.5f,1};int rotate[4]={1,2,1,1};
 for(int i=0;i<120;i++){aw_vehicle_step(&m,&boat,rotate,1.0f/30,&neighbor,1,-1);assert(!aw_bodies_overlap(aw_vehicle_body(&boat),neighbor,0));}assert(boat.contact);
 printf("VEHICLE_TEST astar=PASS fixed_wing_forward_and_turn_bounds=PASS quad_strafe_reverse=PASS sub_medium=PASS oriented_collision=PASS no_penetration=PASS rotation_collision=PASS variant_tradeoffs=PASS\n");
}
