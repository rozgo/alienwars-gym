...
int main(void){
 uint32_t digest=2166136261u;int routes=0,ground=0,naval=0,air=0;
 for(int sym=0;sym<2;sym++)for(int seed_index=0;seed_index<4;seed_index++){
  ...
  for(int unit=0;unit<8;unit++){
   ...
   for(int i=0;i<p->count;i++){
    ...
    if(p->layer==AW_PATROL_AIR){air++;assert(p->altitude[i]>=aw_patrol_roof(&map,p->route[i]%64+.5f,p->route[i]/64+.5f)+6+p->variant*6-.001f);}
   }
   for(int i=0;i<(p->count-1)*8;i++){
    ...
    if(p->layer==AW_PATROL_AIR){assert(a.q>=aw_patrol_roof(&map,a.x,a.z)+5.9f+p->variant*6);assert(aw_density(&map,a.x,a.q,a.z)<0);}
   }
